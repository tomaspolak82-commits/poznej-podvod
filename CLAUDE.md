...
- **Stav k 25. 9. 2026 (konec session):** čeká se na Tomášovu kontrolu textů `zpravy-06` až `zpravy-08` v `docs/navrhy-scenaru-zpravy.md` (číslo +420 772 145 208 a obec Javorná Lhota schválené, tvrzení za firmy ověřená). Věta o 7726 je ve `zpravy-01` a v nápovědě Zpráv nahrazená. Několik lokálních commitů **není pushnutých** (poslední pushnutý je `ce6fda8`). Push a nasazení až spolu se zabudovanými texty 06–08, předtím celá sada testů.
- **Další krok:** po schválení zabudovat `zpravy-06` až `zpravy-08` (nahradí testovací `zpravy-06` a `zpravy-07`), celá sada, push, nasazení. Pak doplňování obou sekcí na 20 scénářů.

**Otevřené úkoly:**
- **Před zveřejněním odkazu na hru: testovací zprávy.** V bance zůstávají testovací zprávy Zpráv `zpravy-06` a `zpravy-07` (testovací e-maily nahradily v milníku 6 skutečné scénáře). Nasazuje se i s nimi (web má `noindex`, Tomášovo rozhodnutí v milníku 4). Než se odkaz na hru kdekoli zveřejní, musí být všechny odstraněné a nahrazené skutečnými scénáři.
